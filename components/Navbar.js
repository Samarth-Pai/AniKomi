  ...
  return (
    <nav className='flex justify-between items-center border-b px-2 md:px-3 fixed z-10 w-full backdrop-blur-2xl bg-amber-950/50'>
      ...
      <div>
        ...
        </> : <><div className='gap-2 m-3 hidden md:flex'>
        ...
        <div className='flex md:hidden relative'>
          <Image src={"/menu.png"} className='invert m-2' alt={"menu png"} width={30} height={30}
          onClick={()=>setShowLoginMenu(!showLoginMenu)}
          >
          </Image>
          {showLoginMenu && <div className='absolute rounded-lg p-2 right-1 top-13 bg-black/80 border'>
              <button className='p-1' onClick={(e) => setShowLoginMenu(false) || router.push(`/signup`)}>
                Signup
              </button>
              <div className='h-[0.5px] bg-amber-50 w-full'></div>
              <button className='p-1' onClick={(e) => setShowLoginMenu(false) || router.push(`/login`)}>
                Login
              </button>
            </div>}
        </div>
